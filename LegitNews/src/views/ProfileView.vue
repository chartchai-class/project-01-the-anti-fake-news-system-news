...
<template>
  <div v-if="u" class="max-w-3xl w-full mx-auto bg-white rounded-xl shadow p-6">
    ...
    <div class="mt-4 flex flex-wrap gap-2">
      <button @click="readNotifications" class="px-3 py-2 rounded border">Mark notifications read</button>
      <button @click="applyMembership" class="px-3 py-2 rounded border">Apply for membership</button>
      <RouterLink to="/" class="px-3 py-2 rounded border no-underline text-inherit">Back to Home</RouterLink>
      <button @click="logout" class="ml-auto px-3 py-2 rounded border bg-black text-white">Logout</button>
    </div>
  </div>
