...
const reportedCommentsData = ref([
    { id: 101, content: "This is completely fake news! Don't believe anyt...", author: 'Anonymous User', article: 'Breaking: New Climate Agreement Signed', date: '2025-10-27', reports: 5, reason: 'Spam/Harassment' },
    { id: 102, content: "Offensive comment with inappropriate languag...", author: 'John Troll', article: 'Tech Giant Announces New Innovation', date: '2025-10-26', reports: 8, reason: 'Offensive Content' },
    { id: 103, content: "Click here for free money: suspicious-link.com", author: 'Spammer123', article: 'Local Sports Team Wins Championship', date: '2025-10-25', reports: 12, reason: 'Spam/Scam' },
    { id: 104, content: "Hate speech and discriminatory remarks...", author: 'BadUser', article: 'Celebrity Spotted with Alien', date: '2025-10-24', reports: 15, reason: 'Hate Speech' },
    { id: 105, content: "Spreading misinformation deliberately...", author: 'FakeNewsBot', article: 'Miracle Cure Found for All Diseases', date: '2025-10-23', reports: 6, reason: 'Misinformation' },
]);

const deletedHistoryData = ref([
    { id: 201, content: "Previously deleted spam comment", author: 'SpamUser', article: 'Old News Article', deletedBy: 'Admin', deletedDate: '2025-10-20', reason: 'Spam' },
    { id: 202, content: "Another removed offensive comment", author: 'TrollUser', article: 'Another Article', deletedBy: 'Admin', deletedDate: '2025-10-18', reason: 'Offensive Content' },
]);

const commentsToShow = computed(() => {
    return currentTab.value === 'deleted' ? deletedHistoryData.value : reportedCommentsData.value;
});
...
const filteredComments = computed(() => {
    ...
    return commentsToShow.value.filter(comment =>
        comment.content.toLowerCase().includes(term) ||
        comment.author.toLowerCase().includes(term) ||
        comment.article.toLowerCase().includes(term)
    );
});
...
function handleViewComment(commentId) {
    console.log(`Viewing comment details for ID: ${commentId}`);
}

function handleDeleteComment(commentId, reason) {
    const index = reportedCommentsData.value.findIndex(c => c.id === commentId);

    if (index !== -1) {
        const comment = reportedCommentsData.value[index];
        console.log(`Deleting comment ID ${commentId} with reason: ${reason}`);
        
        deletedHistoryData.value.unshift({
            id: comment.id,
        ...
        reportedCommentsData.value.splice(index, 1);
    }
}
...
<div class="p-4 sm:p-8 bg-gray-50 min-h-screen font-inter">
    <div class="max-w-[1200px] mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-6 border-b border-gray-200">
            <button
                ...
                :class="[
                    ...
                    currentTab === 'reported' ? 'text-gray-900 border-b-2 border-gray-900' : 'text-gray-500 hover:text-gray-700'
                ]"
            >
                Reported Comments
        ...
        <div class="mb-6" v-if="currentTab === 'reported'">
            <div class="relative">
                ...
                <input
                    type="text"
                    v-model="searchQuery"
                    placeholder="Search comments by content, author, or news title..."
        ...
        <div class="mb-6" v-else>
            <div class="relative">
                ...
                <input
                    type="text"
                    v-model="searchQuery"
                    placeholder="Search deleted comments by content, author, or reason..."
                    class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition duration-150"
                />
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-if="filteredComments.length === 0">
                        <td :colspan="currentTab === 'reported' ? 7 : 7" class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
